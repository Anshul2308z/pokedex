...
export async function commandCatch(state, ...pokemons) {
    ...
    if (caught && !state.pokedex[name]) {
        ...
        const statsByName = Object.fromEntries(data.stats.map((s) => [s.stat.name, s.base_stat]));
